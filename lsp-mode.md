...
for my gradle based tests with lsp-mode so it was green field config.  I hoped that dape, the
dap-mode equivalent for eglot, would just work but that turned out to not be the case.  None of the
needed bundle classes were registered with jdtls, it couldn't find any of the tests and there was no
real provision to report results.  It appears the official current workaround in all the packages I
examined is to launch the unit test in a subprocess via the junit standalone console jar with a
debug port open and then attach. So I burnt some tokens on this problem to deal with digging through
a fairly complex protocol I didn't want to spend time understanding myself.  What I have now is a
fairly complex set of java that properly sets all of the above up
	
1. Bundle of jars needed within jdtls to support dap. This is fairly complex and a potential point of failure.
2. Decode the results protocol so test summaries can be generated.
...
there that does any of this and if I find something I can leverage long term I'll cutover. I think
that adding the gradle classpath calculation may unblock the junit standalone console based
solutions.  So the question remains whether its worth the complexity to run fully natively in DAP.
